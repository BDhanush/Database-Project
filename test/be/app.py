...
def order(data,access_code,payment_reference):
    ...
    for i in data:
        ...
        # Check if special instructions are provided and if they exceed the maximum allowed length
        special_instructions = i.get("special_instructions") or None # If no special instructions, set to None
        if special_instructions and len(special_instructions) > 100:
            raise ValueError("Special instructions exceed the maximum allowed length")
        email = i.get("email")
