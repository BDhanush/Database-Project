...
interface CartContextProps {
  ...
  updateCartItem: (menuItemId: number, updates: Partial<CartItem>) => void; // function to update cart items
}
...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const updateCartItem = (menuItemId: number, updates: Partial<CartItem>) => {
    setCart(prevCart =>
      prevCart.map(item =>
        item.menu_item_id === menuItemId ? { ...item, ...updates } : item
      )
    );
  };
